...
This document takes in the raw data from the original NHM sources and does some first-pass filterring steps. 

# Loading Data
Loading and cleaning data from original data sources: Wilson et al https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.13844 , 
Sourced from: 
https://zenodo.org/records/5759759
and
https://zenodo.org/records/6066070 for the measurements. 

and for the locations: Paterson, Albuquerque, Blagoderov, Brooks et al. (2016). iCollections [Data set]. Natural History Museum. https://doi.org/10.5519/0038559  https://doi.org/10.5519/qd.tqng8ri6. 
...
NHM_data <- read_csv("C:/Users/pemb4504/Documents/7ba5ee8575924a906078531b9212aec880045725/resource.csv") ## large file not included in dataset -> available from the doi above. 
NHM_data %>%
...
Various self-explanatory bounding box, very rare species, and obvious error filtering steps. Where wings differ by more than 2mm, assume either damage or AI errors and removed. 
...
NHM_filter1 %>%
  ...
  filter( !is.na(`Taxon name`))-> NHM_filter2  ## filter out those vagrants etc that are not in the name table because they are not going to be analysed
...
read_csv('../data/brood_table.csv') %>%
  left_join(read_csv('../data/names_table.csv'), by = c('NHM name', 'Taxon name'))%>%
  select( `Taxon name`,
         maximum_plausible,
...
## Dealing with odd month windows by picking middle
...
```{r}
write_csv(NHM_tidy, '../data/NHM/NHM_measurements_clean.csv')
```
