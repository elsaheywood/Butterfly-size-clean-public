...
```{r}
NHM_records <- read_csv('../data/NHM/NHM_measurements_clean.csv')
```
...
## Extracting all years out into a table of specimen x months
...
for(i in WeirdCoastal_Numbs){   ## takes a little while to run (~1 minute)
  ...
  for(m in 1:length(Months)){
    MonthLayer = Long_Lats_Years[[i,Months[m]] ]
...
write_csv(Long_Lats_Years, 
          '../data/NHM/NHM_Long_Lats_Years.csv')
```


```{r}
Long_Lats_Years <- read_csv('../data/NHM/NHM_Long_Lats_Years.csv')
...
NHM_records %>%
  ...
  write_csv('../data/NHM/NHM_w_temps_Monthly.csv')
...
VC_Monthly_Temps<- exactextractr::exact_extract(Collated_MonthlyTemps,
                                                 ViceCounties, fun = 'mean', weights ='area',
                                                 append_cols = c( 'VCNUMBER','VCNAME' ))

write_csv(VC_Monthly_Temps, '../data/VC_Monthly_Temps.csv' )
```

```{r}
VC_Monthly_Temps<- read_csv('../data/VC_Monthly_Temps.csv' )
...
```{r}
OUMNH_data <- read_csv('../data/OUMNH/OUMNH_measurements_clean.csv')
...
write_csv(OUMNH_data_temps, '../data/OUMNH/OUMNH_data_temps_Monthly.csv')
