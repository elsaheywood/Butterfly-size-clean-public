...
Main document for making main text results for the OUMNH data and other comparisons.  Model fits at this level are saved in `TopLevelModelFits/`. At the end, also includes comparison between coefficients from different datasets. 
...
Joined_coefficients %>%
  ...
  filter( !is.na(mean_OUMNH ))-> JoinedDatadf


Compare1<- JoinedDatadf%>%
  filter(variable == 'b_Latitude')%>%
  ...
  geom_point()+
  theme_bw()+
  coord_fixed(ylim = c( -0.025, 0.03),
              xlim = c( -0.1, 0.1), ratio = 1)+
  xlab('OUM Estimate')+ylab('NHM Estimate')

Compare2<- JoinedDatadf%>%
  ...
  geom_point()+
  theme_bw()+
  coord_fixed(ylim = c( -0.02, 0.01),
              xlim = c( -0.03, 0.03), ratio = 1)+
  xlab('OUM Estimate')+ylab('NHM Estimate')

Compare3<- JoinedDatadf%>%
  ...
  geom_point()+
  theme_bw()+
  coord_fixed(ylim = c( -0.05, 0.11),
              xlim = c( -0.15, 0.15), ratio = 1)+
  xlab('OUM Estimate')+ylab('NHM Estimate')


plot_grid(Compare1, Compare2,Compare3, nrow=3,
          labels = c( 'a) Latitude Coefficient',
                      'b) Temperature Coefficient',
                      'c) Year Coefficient'),hjust = -0.1,
          rel_heights = c( 1.5,2,2))

ggsave(filename = '../figs/compare_coefs_across_datasets.png', width = 6, height =12)
